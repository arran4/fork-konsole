...
QuickCommandsWidget::QuickCommandsWidget(QWidget *parent)
    : QWidget(parent), ui(std::make_unique<Ui::QuickCommandsWidget>()),
  ...
  connect(ui->btnAdd, &QPushButton::clicked, this,
          &QuickCommandsWidget::addMode);
  connect(ui->btnSave, &QPushButton::clicked, this,
          &QuickCommandsWidget::saveCommand);
  connect(ui->btnUpdate, &QPushButton::clicked, this,
          &QuickCommandsWidget::updateCommand);
  connect(ui->btnCancel, &QPushButton::clicked, this,
          &QuickCommandsWidget::viewMode);
  connect(ui->btnRun, &QPushButton::clicked, this,
          &QuickCommandsWidget::runCommand);

  connect(ui->invertFilter, &QPushButton::clicked, priv->filterModel,
          &FilterModel::setInvertFilter);
  ...
  connect(ui->commandsTreeView, &QTreeView::doubleClicked, this,
          &QuickCommandsWidget::invokeCommand);
  connect(ui->commandsTreeView, &QTreeView::clicked, this,
          &QuickCommandsWidget::indexSelected);

  connect(ui->commandsTreeView, &QTreeView::customContextMenuRequested, this,
          &QuickCommandsWidget::createMenu);

  connect(&priv->shellCheckTimer, &QTimer::timeout, this,
          &QuickCommandsWidget::runShellCheck);
  connect(ui->command, &QPlainTextEdit::textChanged, this,
  ...
  const QString entry =
      settings.value(QStringLiteral("shortcut"), defText).toString();
...
void QuickCommandsWidget::indexSelected(const QModelIndex &idx) {
  ...
  const auto sourceIdx =
      priv->filterModel->mapToSource(ui->commandsTreeView->currentIndex());
  ...
  if (item != nullptr && item->parent() != nullptr) {
    const auto data = item->data(QuickCommandsModel::QuickCommandRole)
                          .value<QuickCommandData>();
...
void QuickCommandsWidget::invokeCommand(const QModelIndex &idx) {
  ...
  const auto data = item->data(QuickCommandsModel::QuickCommandRole)
                        .value<QuickCommandData>();
  priv->controller->session()->sendTextToTerminal(data.command,
                                                  QLatin1Char('\r'));
...
void QuickCommandsWidget::setCurrentController(
...
void QuickCommandsWidget::createMenu(const QPoint &pos) {
  ...
  const bool isParent =
      sourceIdx.parent() == priv->model->invisibleRootItem()->index();
...
void QuickCommandsWidget::runShellCheck() {
  ...
  if (errorString.isEmpty() && ui->warningMessage->isVisible()) {
    ui->warningMessage->animatedHide();
  }
